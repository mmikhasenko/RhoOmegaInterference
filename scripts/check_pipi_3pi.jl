...
p1 = let
    plot(xlab=L"m_{\pi\pi}\,\,(\mathrm{GeV})",
        title=L"|A(\pi\pi \to \pi\pi)|^2", leg=:topleft)
    f(s) = I_X2ππ(s; α=0.0)
    plot!( e->f(e^2)/f(mρ^2-1e-4), 2mπ, 0.8, lab="K-matrix")
    # 
    f(s) = abs2(BW_Pwave(s; R=Nominal.R, endep=:rho))
    plot!(e->f(e^2)/f(mρ^2-1e-4), 2mπ, 0.8, lab="BW")
end
p2 = let
    plot(xlab=L"m_{3\pi}\,\,(\mathrm{GeV})",
        title=L"|A(3\pi \to 3\pi)|^2", leg=:topleft)
